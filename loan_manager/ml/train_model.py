...
def add_features(X: pd.DataFrame) -> pd.DataFrame:
    ...
    X['Monthly_Payment'] = (X['LoanAmount'] * 0.08 / 12) * (1 + 0.08 / 12) ** X['Loan_Amount_Term'] / ((1 + 0.08 / 12) ** X['Loan_Amount_Term'] - 1)
...
def train(csv_path: Path):
    ...
    metrics = {
        ...
        "feature_importance": dict(zip(X.columns, best_model.named_steps['classifier'].feature_importances_))
    }
